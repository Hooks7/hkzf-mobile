...
export default class RentAdd extends Component {
	...
	onSelect = (val) => {
		this.setState({ supporting: val.join('|') });
	};
	...
	addHouse = async () => {
		...
		let res = await request.post('/houses/image', formData, {
			headers: {
				'Content-Type': 'multipart/form-data'
			}
		});

		if (res.status === 200) houseImg = res.data.body.join('|');

		// 发布房源
		community = community.id;

		let result = await request.post('/user/houses', {
			title,
			description,
			houseImg,
			oriented,
			...
			<div className={styles.root}>
				...
				<List className={styles.header} renderHeader={() => '房源信息'} data-role="rent-list">
					...
					<Picker
						...
						cols={1}
						onChange={(val) => {
							this.getFormVal('roomType', val[0]);
						}}
					>
					...
					<Picker
						...
						cols={1}
						onChange={(val) => {
							this.getFormVal('oriented', val[0]);
						}}
					>
				...
				<List className={styles.desc} renderHeader={() => '房屋描述'} data-role="rent-list">
					<TextareaItem
						...
						value={description}
						onChange={(val) => {
							this.getFormVal('description', val);
						}}
					/>
				</List>
